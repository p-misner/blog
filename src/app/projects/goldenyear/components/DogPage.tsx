...
function DogPage(props: DogProps) {
  const { dogName, nextDog, timelineRecords, record } = props;
    ...
    <S.Background>
      <S.Content ref={widthRef}>
        ...
        <S.Blocks style={{ paddingTop: "24px" }}>
          <S.LeftBlock>
            {record?.dogSvg && (
              <S.DogImage src={record?.dogSvg} alt="handdrawn" />
            )}
            <S.VerticalBlocks>
              <S.TitleItalics> foster {record?.fosterNum}</S.TitleItalics>
          ...
          <S.RightBlock>
            <S.DescripPara>
              {record?.summaryText &&
                record?.summaryText.map((x: any, i: number) =>
                  x.type === "bold" ? (
        ...
        <S.ColumnContent>
          <S.HeroDiv>
            <S.HeroText>
              {record?.heroText &&
                record?.heroText.map((x: any, i: number) =>
                  x.type === "text" ? (
                    x.text
                  ) : (
                    <Tooltip
          ...
          {record?.gallery && (
            <S.PictureScroll>
              {record?.gallery.map((x: any, i: number) => (
                <Tooltip
                  key={
                    x.tooltipText +
          ...
          <S.TextDiv>
            {record?.overflowText &&
              record?.overflowText.map((x: any, i: number) => (
                <S.OverflowPara
          ...
          {nextDog !== "Timeline" && (
            ...
            >
              <path
                ...
                stroke="#333333"
              />
              <Link href={`/projects/goldenyear/${nextDog.replace(" ", "")}`}>
                <text
                  fill="#000"
                  x="45"
                  y="73"
          ...
          >
            ...
            >
              ...
              {timelineRecords.map((x: TimelineRecordsProp, i: number) => (
                <Link
                  key={x.dogName}
                  href={`/projects/goldenyear/${x.dogName.replace(" ", "")}`}
                >
                  <rect
                    ...
                    height="30"
                    fill={x.dogName === dogName ? "url(#golden)" : "url(#gray)"}
                    stroke="black"
