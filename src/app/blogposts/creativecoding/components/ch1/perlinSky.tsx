...
export const PerlinSky = () => {
    ...
    for (let x = 0; x < p5.width; x++) {
      for (let y = 0; y < p5.height; y++) {
        ...
        let brightness = p5.map(
          p5.noise(x * 0.005, y * 0.005, time),
          0,
          1,
          100,
          255,
        );
        let hue = p5.map(p5.noise(x * 0.005, y * 0.005, time), 0, 1, 130, 180);

        // p5.pixels[index + 0] = brightness;
        // p5.pixels[index + 1] = brightness;
        // p5.pixels[index + 2] = 230;

        p5.set(x, y, p5.color(hue, 80, brightness));
  ...
  return (
    <CanvasWrapper color={[0, 255, 0, 1]}>
      <CanvasTitle>perlin color </CanvasTitle>
      <CanvasOutlinep5
        ref={containerRef}
        $active={inDiv}
